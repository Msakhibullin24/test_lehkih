...
Multi-GPU через DistributedDataParallel (torchrun).

Запуск:
    1 GPU:   python train.py
    2+ GPU:  torchrun --nproc_per_node=2 train.py

Структура проекта:
    LUNA16_Project/
    ├── data/              ← subset0..subset9 внутри
...
def is_main_process():
    """Только главный процесс (rank 0) печатает и сохраняет."""
    return not dist.is_initialized() or dist.get_rank() == 0


def setup_ddp():
    """Инициализация DDP, если запущен через torchrun."""
...
def main():
    ...
    # DDP SETUP
    # =========================================================================
    use_ddp, rank, local_rank = setup_ddp()
    ...
    if not torch.cuda.is_available():
        ...
        # Информация обо всех GPU (только rank 0)
        total_gpu_mem = 0.0
        ...
        if per_gpu_mem >= 20:       # A100 40/80GB, RTX 3090 24GB etc.
            ...
            MAX_EPOCHS = 80
            BATCH_SIZE = 1           # per-GPU batch (DDP: каждая GPU получает свой)
            ACCUM_STEPS = 4          # Эффективный batch = 1 * num_gpus * 4
            PATCH_SIZE = (96, 96, 96)
            NUM_SAMPLES = 4
            NUM_WORKERS = 4
    ...
    effective_batch = BATCH_SIZE * ACCUM_STEPS * (num_gpus if use_ddp else 1)
    if is_main_process():
        mode_str = f"DDP на {num_gpus} GPU" if use_ddp else f"GPU: {num_gpus}"
        print(f"Устройство: {device} ({mode_str})")
        print(f"Патчи: {PATCH_SIZE}, Batch per GPU: {BATCH_SIZE}, "
              f"accum: {ACCUM_STEPS}, effective: {effective_batch}, Эпохи: {MAX_EPOCHS}")
    ...
    # DDP: DistributedSampler шардит данные между GPU
    ...
    train_loader = DataLoader(
        ...
        shuffle=(train_sampler is None),   # shuffle только если нет sampler
    ...
    if use_ddp:
        ...
        if is_main_process():
            print(f"✓  DDP: модель распределена на {num_gpus} GPU")
    ...
    # LR масштабируется на количество GPU при DDP
    effective_lr = LEARNING_RATE * (num_gpus if use_ddp else 1)
    ...
    for epoch in range(MAX_EPOCHS):
        ...
        # DDP: обновляем семпл для каждой эпохи
        ...
        for batch_data in train_pbar:
            ...
            # Конвертация MetaTensor → обычный Tensor (избегаем баги с DDP/AMP)
        ...
        if (epoch + 1) % VAL_INTERVAL == 0 or (epoch + 1) == MAX_EPOCHS:
            model.eval()
            # sliding_window_inference — используем raw-модель (без DDP обёртки)
            ...
            with torch.no_grad():
                ...
                for val_data in val_pbar:
                    ...
                    if isinstance(val_inputs, MetaTensor):
                        val_inputs = val_inputs.as_tensor()
        ...
        # Периодическое сохранение чекпоинтов (только rank 0)
        if is_main_process() and (epoch + 1) % SAVE_INTERVAL == 0:
            state_dict = model.module.state_dict() if hasattr(model, 'module') else model.state_dict()
        ...
        # Синхронизация всех GPU перед следующей эпохой
        if use_ddp:
            dist.barrier()
